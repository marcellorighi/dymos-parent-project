...
class DroneODE(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):  # ← INDENTED (inside class)
        ...
        for i, (ax, ay, az) in enumerate(avoid_points):
            ...
            penalty = penalty_strength * (1. + 0.01*(acc_x**2 + acc_y**2 + acc_z**2)) / dist2 
    ...
    def compute_partials(self, inputs, partials):  # ← INDENTED (inside class)
        ...
        for i, (ax, ay, az) in enumerate(avoid_points):
            ...
            dpenalty_dax += penalty_strength * 0.01 * 2 * acc_x /dist2 
            dpenalty_day += penalty_strength * 0.01 * 2 * acc_y /dist2
            dpenalty_daz += penalty_strength * 0.01 * 2 * acc_z /dist2
...
p.set_val('traj.phase0.states:y', phase.interp('y', [0, 500]))

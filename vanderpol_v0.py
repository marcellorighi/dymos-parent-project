...
class VanderpolODE(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['x0dot'] = (1.0 - x1**2) * u *x0 - x1
        outputs['x1dot'] = x0
    ...
    def compute_partials(self, inputs, jacobian):
        ...
        jacobian['x0dot', 'x1'] = -2.0 * x1 * u * x0  - 1 # x0 - 1.0
...
def vanderpol(transcription='gauss-lobatto', num_segments=15, transcription_order=3,
    ...
    phase.set_control_val('u', +1.50)

    return p

# Create the Dymos problem instance
p = vanderpol(transcription='gauss-lobatto', use_pyoptsparse=True, optimizer='IPOPT', num_segments=96)
...
plt.plot(t_sim, x1_sim, label='Simulation (Continuous)', color='blue', linewidth=2)
...
plt.subplot(2, 2, 2)
plt.plot(t_sim, x0_sim, label='Simulation (Continuous)', color='blue', linewidth=2)
...
plt.subplot(2, 2, 3)
plt.plot(x0_sim, x1_sim, label='Simulation (Continuous)', color='blue', linewidth=2)

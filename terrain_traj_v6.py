...
def generate_urban_terrain(seed = None, grid_size=150, area_dim=500, num_hills=12, num_clusters = 200, buildings_per_cluster = 1, hill_min_height = 20, hill_max_height=160, hill_min_width = 60., hill_max_width = 360.):
    ...
    grad_x = 0.05 
    grad_y = 0.02
    Z = grad_x * X + grad_y * Y #np.zeros_like(X)
    ...
    cluster_spread = 375  # How far buildings stray from the cluster center

    sensitive_points = []

    for _ in range(num_clusters):
        # 1. Pick a "Parent" center for the cluster
        parent_x, parent_y = np.random.uniform(-area_dim*0.7, area_dim*0.7, 2)

        along_diag = np.random.uniform(-area_dim*0.85, area_dim*0.6)
        off_diag = np.random.uniform(-area_dim*0.25, area_dim*0.25)
        # parent_x = np.random.uniform(-area_dim*0.8, 0.)
        # parent_y = np.random.uniform( 0., area_dim*0.8)
        ...
        for _ in range(buildings_per_cluster):
            ...
            cx = along_diag + off_diag + np.random.uniform(0, cluster_spread)
            cy = along_diag - off_diag + np.random.uniform(0, cluster_spread)
...
class DroneTerrainODE(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # 4. Acoustic annoyance / sound power 
        points = self.options['avoid_points']
        thrust_mag = np.sqrt(inputs['ax']**2 + inputs['ay']**2 + (inputs['az'] + 9.81)**2) / 9.81
        dist2 = np.zeros_like(inputs['x']) 
        rho = 10.
        noise = []
        for px, py, pz in points:
            noise_point = 100 + 20. * np.log10( (1. / ( (inputs['x'] - px)**2 + (inputs['y'] - py)**2 + (inputs['z'] - pz)**2 + 1.e-2)) ) + 25 * np.log10(thrust_mag)            
...
phase.add_state('annoyance', 
                rate_source='noise_pl', 
                fix_initial=True, 
                fix_final=False,
                lower = 0, 
                ref=1000.0, 
...
phase.add_path_constraint('clearance', lower= 9.0, upper = 120, ref= 9.0)

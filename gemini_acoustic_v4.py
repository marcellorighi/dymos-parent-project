...
def generate_urban_terrain(grid_size=100, area_dim=500, num_hills=3, num_buildings=10):
    ...
    # Add "buildings" (steeper, narrower Gaussians)
    for _ in range(num_buildings):
        cx, cy = np.random.uniform(-area_dim*0.8, area_dim*0.8, 2)
        height = np.random.uniform(10, 40)
        width = np.random.uniform(5, 15) # Narrow width = building-like
        Z += height * np.exp(-((X - cx)**2 + (Y - cy)**2) / (2 * width**2))
...
# Generate the data
x_coords, y_coords, z_map = generate_urban_terrain()

# Create the interpolator for use in your ODE
terrain_interp = RegularGridInterpolator((x_coords, y_coords), z_map, method='cubic')

...
class DroneODE(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        for var in ['x_dot', 'y_dot', 'z_dot', 'vx_dot', 'vy_dot', 'vz_dot', 'inst_penalty', 'acc_mag2']: #, 'energy']:
    ...
    def compute(self, inputs, outputs):
        ...
        #outputs['energy'] = np.sqrt(v_mag3 **2)  

# --- Setup Problem ---
p = om.Problem()
p.driver = om.pyOptSparseDriver(optimizer='IPOPT')
p.driver.opt_settings['print_level'] = 5
p.driver.opt_settings['delta'] = 1e-2
p.driver.opt_settings['max_iter'] = 400

# Generate Obstacles
avoid_points = [(random.uniform(50, 450), random.uniform(50, 450), 0) for _ in range(50)]

traj = dm.Trajectory()
phase = dm.Phase(ode_class=DroneODE, 
                 ode_init_kwargs={'avoid_points': avoid_points},
                 transcription=dm.GaussLobatto(num_segments=8, order=5))
p.model.add_subsystem('traj', traj)
traj.add_phase('phase0', phase)

# Time, States, and Controls
phase.set_time_options(fix_initial=True, fix_duration=False, duration_bounds=(5, 100))

# States: x, y, z are fixed at start (0,0,0) and end (500,500,10)
for s in ['x', 'y', 'z']:
    phase.add_state(s, fix_initial=True, fix_final=True, rate_source=f'{s}_dot')

# Velocities: Fixed at 0 at start/end
for v in ['vx', 'vy', 'vz']:
    phase.add_state(v, fix_initial=True, fix_final=True, rate_source=f'{v}_dot')

phase.add_state('acc_integral', rate_source='acc_mag2', fix_initial=True)
...
for a in ['ax', 'ay', 'az']:
    phase.add_control(a, lower=-5.0, upper=5.0, rate_continuity=True, 
                  rate2_continuity=False)

# INTEGRATE PENALTY: This creates the "Accumulated Reward" automatically
phase.add_state('total_penalty', rate_source='inst_penalty', fix_initial=True, fix_final=False)
...
class ObjectiveComp(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        # outputs['J'] = inputs['time'] + 20.0 * inputs['penalty'] + 0.0 * inputs['acc_integral'] + 0.0 * inputs['energy_final']
...
# p.set_val('traj.phase0.states:energy_spent', 0.0)

...
# p.driver.options['optimizer'] = 'CONMIN'   # 'IPOPT'  # or 'SNOPT'
# p.driver.options['optimizer'] = 'SLSQP'   # 'IPOPT'  # or 'SNOPT'
p.driver.options['optimizer'] = 'IPOPT'  # or 'SNOPT'

# 3. Create trajectory and phase
traj = dm.Trajectory()
p.model.add_subsystem('traj', traj)

avoid_points = [(10, 10, 2), (20, 20, 6), (30, 5, 10)]  # Points to avoid
...
phase.add_state('x', fix_initial=True, fix_final=True, 
                ...
                ref=50.0,      # ← Expected magnitude
                defect_ref=10.0)  # ← Defect scaling
...
phase.add_state('z', fix_initial=True, fix_final=True,
                ...
                ref=20.0,      # ← Different from x,y
                defect_ref=5.0)


# Instead of fix_initial=True for all velocities:
phase.add_state('vx', fix_initial=False, fix_final=False,  # ← Changed
                rate_source='vx_dot', units='m/s',
                ref=5.0, defect_ref=1.0)

phase.add_state('vy', fix_initial=False, fix_final=False,  # ← Changed
                rate_source='vy_dot', units='m/s',
                ref=5.0, defect_ref=1.0)

phase.add_state('vz', fix_initial=False, fix_final=False,  # ← Changed
                rate_source='vz_dot', units='m/s',
                ref=5.0, defect_ref=1.0)

# Add bounds instead of fixing
phase.set_state_options('vx', lower=-5, upper=5, initial_bounds=(0, 0))
phase.set_state_options('vy', lower=-5, upper=5, initial_bounds=(0, 0))
phase.set_state_options('vz', lower=-5, upper=5, initial_bounds=(0, 0))


# Controls with scaling
phase.add_control('ax', lower=-5, upper=5, units='m/s**2',
                  ref=5.0)   # ← Scale to expected magnitude
...
phase.add_path_constraint('v_squared', upper=25.0, units='m**2/s**2',
                         ref=25.0)  # ← Scale constraint

phase.add_path_constraint('dist_to_avoid', lower=2.0, units='m',
                         ref=10.0)  # ← Scale to typical distance

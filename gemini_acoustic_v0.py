...
class DroneODE(om.ExplicitComponent):
    def initialize(self):
        ...
        # self.options.declare('sensitive_points': list of tuples (x, y, limit_dB, annoyance_weight)
        self.options.declare('sensitive_points', types=list) 
        self.options.declare('rho', default=10.0) # KS scaling parameter
    ...
    def setup(self):
        ...
        for var in ['x_dot', 'y_dot', 'z_dot', 'vx_dot', 'vy_dot', 'vz_dot', 'inst_penalty', 'acc_mag2', 'energy', 'wind_x', 'wind_y', 'wind_z', 'ks_norm_Lp', 'ks_annoyance']:
    ...
    def compute(self, inputs, outputs):
        ...
        for px, py, limit_Lp, weight in points:
            # 1. Geometry
            dist = np.sqrt((x - px)**2 + (y - py)**2 + z**2)
    ...
    #     val = partials['acc_mag2', 'x'][0]
    #     print(val) 
    #     val = partials['energy', 'y'][0]
    #     print(val) 
    #     val = partials['energy', 'z'][0]
    #     print("Z",val) 
...
# Format: (x, y, limit_dB, annoyance_weight)
landmarks = [
    (100.0, 100.0, 55.0, 3.0),  # Hospital (Very Quiet, High Priority)
    (400.0, 100.0, 60.0, 2.0),  # Primary School (Quiet)
    (250.0, 450.0, 75.0, 0.5),  # Industrial Zone (Loud permitted, Low Priority)
]

# --- Part B: Define Residential Grid ---
residential_points = []
# Create a 5x5 grid covering the "Main Street" residential block
grid_x = np.linspace(150, 350, 5)
grid_y = np.linspace(200, 350, 5)

for px in grid_x:
    for py in grid_y:
        # Standard residential limit (65dB) and baseline weight (1.0)
        residential_points.append((px, py, 65.0, 2.0))
...
phase.add_state('z', fix_initial=True, lower = 0, upper = 120., ref=5.0, fix_final=True, rate_source=f'z_dot')
...
class ObjectiveComp(om.ExplicitComponent):
    def setup(self):
        ...
        self.add_input('total_annoyance', units=None) 
        self.add_output('J')
    ...
    def compute(self, inputs, outputs):
        ...
        outputs['J'] = 0.002 * ( inputs['time'] + 0.0 * inputs['penalty'] + 2.0 * inputs['acc_integral'] + 0.001 * inputs['energy_final'] ) + 0.075*inputs['total_lp_dose']
...
p.set_val('traj.phase0.states:z', phase.interp('z', [0, 0]))
